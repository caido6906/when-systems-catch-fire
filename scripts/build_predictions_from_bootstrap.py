...
def render_prediction_page(prediction: dict) -> str:
    ...
    lines = [
        f"# {prediction['id']}｜{prediction['title']['zh']} / {prediction['title']['en']}",
        "",
...
def render_category_page(category: dict) -> str:
    ...
    lines = [
        ...
        "",
        f"English: This page collects testable prediction entrances formed by the Ignition framework in the {category['title']['en']} problem domain. It is not a conventional textbook, but a future-facing structure generated by functions, cases, discoveries, and bootstrap cycles within this domain.",
        "",
    ...
    if category.get("related_functions"):
        for row in category["related_functions"][:30]:
            lines.append(f"- [{row['id']}｜{row['title']['zh']} / {row['title']['en']}]({rel_link(current_path, REPO_ROOT / row['page'])})")
    ...
    if curated:
        for pred in curated:
            lines.append(f"- [{pred['id']}｜{pred['title']['zh']} / {pred['title']['en']}]({rel_link(current_path, REPO_ROOT / pred['page'])})")
    else:
...
def render_index(predictions: list[dict], categories: list[dict]) -> str:
    ...
    for category in categories:
        ...
        coverage = category["coverage"]
        lines.append(
            f"| [{category['title']['zh']} / {category['title']['en']}]({rel_link(PREDICTIONS_HUMAN_MD, REPO_ROOT / category['page'])}) | {coverage['curated_predictions_count']} | {coverage['prediction_leads_count']} | {coverage['related_functions_count']} functions, {coverage['related_cases_count']} cases, {coverage['related_discoveries_count']} discoveries |"
        )
    lines.extend(
        [
            "",
...
def render_bootstrap_report(category_map: list[dict]) -> str:
    ...
    for entry in category_map:
        if entry["id"] == "other":
            continue
        lines.append(
            f"| {entry['title']['zh']} / {entry['title']['en']} | {entry['coverage']['related_functions_count']} | {entry['coverage']['related_cases_count']} | {entry['coverage']['related_discoveries_count']} | {entry['coverage']['curated_predictions_count']} |"
        )
    lines.extend(
        [
            "",
